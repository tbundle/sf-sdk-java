...
public abstract class SmilieProcessor {
	public static final String SMILIE_ENCODER = "&smilie%s;";
	public static final String SMILIE_DECODER = "&smilie";
	public static final String SMILIE_REGEX = "(&smilie(([!@#\\$%\\^\\?&=>\\-<:\\)\\(\\{\\}\\[\\]\\*\\w\"'\\.])+);)";

	private Pattern smilieRegex;
	private SmilieSupplier smilieSupplier;

	public SmilieProcessor(SmilieSupplier supplier) {
		this.smilieSupplier = supplier;
		this.smilieRegex = Pattern.compile(SMILIE_REGEX);
	}

	/**
	 * Encode smilie code
	 * 
	 * @param code smilie code
	 * @return encoded string
	 */
	public String encode(String code) {
		return String.format(SMILIE_ENCODER, code);
	}

	/**
	 * Decode encoded smilie string
	 * 
	 * @param encodedSmilie encoded smilie string
	 * @return decoded smilie code
	 */
	public String decode(String encodedSmilie) {
		Matcher smilieMatcher = this.smilieRegex.matcher(encodedSmilie);
		return smilieMatcher.find() ? smilieMatcher.replaceAll("$2") : encodedSmilie;
	}

	public List<String> tokenizeSmilie(String encodedSmilie) {
		Matcher smilieMatcher = this.smilieRegex.matcher(encodedSmilie);
		if (smilieMatcher.find()) {
			String tokens[] = smilieMatcher.replaceAll("&smilie$2&smilie").split("&smilie");
			ArrayList<String> tokenizedSmiles = new ArrayList<>();
			for (String token : tokens) {
				tokenizedSmiles.add(XMLUtils.decodeSpecialChars(token));
			}
		}
		return Arrays.asList(XMLUtils.decodeSpecialChars(encodedSmilie));
	}

	public SmilieSupplier getSmilieSupplier() {
		return smilieSupplier;
	}

	public void setSmilieSupplier(SmilieSupplier smilieSupplier) {
		this.smilieSupplier = smilieSupplier;
	}
