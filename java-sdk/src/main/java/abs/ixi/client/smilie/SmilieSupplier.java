...
public interface SmilieSupplier {
	/**
	 * Its is a smilie lookup method. SDK uses to lookup smilie binary for a
	 * code. It is upto the implementation to keep the binaries in memory or on
	 * disk.
	 * <p>
	 * For faster access, smilie banaries can be kept in memeory; however in
	 * that case the banary size should not be large.
	 * </p>
	 * 
	 * @param code smilie code
	 * @return if code found, it returns binary data for the smilie image
	 *         otheriwise null
	 */
	public byte[] getSmilieBin(String code);
