...
public class OutboundFileTransferProcessor implements Runnable {
	private static final Logger LOGGER = Logger.getLogger(OutboundFileTransferProcessor.class.getName());
	...
	/**
	 * A queue to hold outbound file transfer requests
	 */
	private LinkedBlockingQueue<OutboundFileTransfer> outboundFileTransferQ;
	...
	/**
	 * Flag to indicate if the {@code OutboundFileTransferProcessor} has
	...
	public OutboundFileTransferProcessor(FileTransferNegotiator negotiator) {
		this.negotiator = negotiator;
		this.outboundFileTransferQ = new LinkedBlockingQueue<>();
		this.running = false;
	...
	public void run() {
		while (!Thread.interrupted()) {
			OutboundFileTransfer transfer = null;
			...
			synchronized (this.lock) {
				...
				transfer = this.outboundFileTransferQ.poll();
			...
			this.handleOutboundFileTransfer(transfer);
		}
	}

	private void handleOutboundFileTransfer(OutboundFileTransfer transfer) {
		LOGGER.fine("handaling outbound file transfer for fileId : " + transfer.getFileId() + " and to : "
				+ transfer.getTo());

		if (this.connection.isReady()) {
			this.startOutboundFileTranfer(transfer);
			...
			try {
				...
				LOGGER.warning("Failed to negotiate file transfer connection for fileId : " + transfer.getFileId()
						+ " due to unknown exception : " + e.getMessage());
				transfer.getCallback().onFailure(FailureReason.UNKNOWN);
			}
		}
	}

	private void startOutboundFileTranfer(OutboundFileTransfer transfer) {
		LOGGER.info("Outbound file transfer started to fileId : " + transfer.getFileId());

		try {
			MultipartWriter writer = new MultipartWriter(connection, transfer);
			writer.write();

			LOGGER.info("Outbound file transfer completed for fileId : " + transfer.getFileId() + " receiver : "
					+ transfer.getTo());
			transfer.getCallback().onSuccess("Media Send Successfully");
		} catch (IOException e) {
			LOGGER.warning("Outbound file transfer failed for fileId : " + transfer.getFileId() + " and receiver : "
					+ transfer.getTo());
			transfer.getCallback().onFailure(FailureReason.TRANSFER_INTERRUPTED);

		} catch (Throwable e) {
			LOGGER.warning("Outbound file interrupted for fileId : " + transfer.getFileId() + " and receiver : "
					+ transfer.getTo());
			e.printStackTrace();
			transfer.getCallback().onFailure(FailureReason.TRANSFER_INTERRUPTED);
		}
	...
	public void submit(OutboundFileTransfer transfer) {
		synchronized (this.lock) {
			this.outboundFileTransferQ.offer(transfer);
