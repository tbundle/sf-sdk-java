...
public interface Connection extends AutoCloseable, Identifiable<String> {
	 ...
	 * 
	 * @return true if connection is establishes successfully
	 * @throws IOException IOException
	 */
	public boolean connect() throws IOException;

	/**
	 * Check if this connection is connected. The method just checks the current
	 * connection state. With the nature of TCP protocol, it may report a false
	 * positive. Without a dial-tone, TCP fails to detect real-time connection
	 * state. Without a FIN signal, socket status may not get updated.
	 * <p>
	 * The method used to determine connection state in multi-threaded
	 * environment; therefore it is advised that implementation of this method
	 * is synchronized on the connection object.
	 * </p>
	 * 
	 * @return true if the state of underlying connection is <i>connected</i>
	 *         else false
	 */
	 ...
	 * 
	 * @param buffer byte buffer which will drained on underlying socket
	 * @return nnumber of bytes written, possibly zero
	 * @throws IOException if IO error occures while writing data
	 */
	public int write(ByteBuffer buffer) throws IOException;

	/**
	 * Write {@link Writable}s onto this connection. It is recommended that this
	 * method be used to write packet onto a connection. A failure in write will
	 * result in socket close.
	 * 
	 ...
	 * {@code Connection}. The method returns time of last activity on this
	 * conenction.
	 * 
	 * @return long long
	 */
