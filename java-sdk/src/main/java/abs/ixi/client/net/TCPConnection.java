...
/**
 * {@code TCPConnection} is a wrapper around {@link SocketChannel}; it
 * represents a physical network connection with dsestination host (server). The
 * class keeps the supplied {@link ConnectionConfig} which is used to reconnect.
 * 
 * @author Yogi
 *
 */
public class TCPConnection implements Connection {
	...
	/**
	 * Constructor to instantiate a {@code TCPConnection} instance The access
	 * modifier is default to force SDK user to use {@link ConnectionManager} to
	 * create a new connection.
	 */
	TCPConnection(ConnectionConfig config) {
	...
	public synchronized boolean connect() throws IOException {
		...
		LOGGER.log(Level.INFO, "Connecting " + this.connConfig.getServerIP() + ":" + this.connConfig.getServerPort());
		this.channel.connect(new InetSocketAddress(this.connConfig.getServerIP(), this.connConfig.getServerPort()));

		// commented due to Android compatibility issue
		// socket.setOption(StandardSocketOptions.TCP_NODELAY, true);
		channel.socket().setTcpNoDelay(true);

		this.isConnected = channel.isConnected();
		...
		try {
			...
			if (bytesRead == -1) {
				...
				try {
					...
					LOGGER.log(Level.FINE, "Caught exception while closing the channel. Ignoring", e);
				}
			}
