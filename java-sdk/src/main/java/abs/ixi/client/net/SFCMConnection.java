...
/**
 * An implementation of {@link TCPConnection} used for transfering files using
 * Stringflow Content Model. The implementation preserves all the sementics of a
 * {@link TCPConnection}; additionally it has a <i>state</i> flag to indicate if
 * the connection is ready for file transfer in either direction.
 * <p>
 * Stringflow Content Model (SFCM) mandates that once there is physical
 * connection established with server, server assigns a SID to the connection
 * and this SID must be communicated to server on an authenticated stream.
 * </p>
 * 
 * @author Yogi
 *
...
public class SFCMConnection extends TCPConnection implements NegotiatedConnection {
	/**
	 * Server assigns SID to every {@code SFCMConnection} instance
	 */
	private String sid;

	/**
	 * Flag to indicate if this connection has been negotiated;
	 * {@code SFCMConnection} can be used for file transfer only when the
	 * negotiation is complete on this connection
	 */
	private volatile boolean negotiated;

	public SFCMConnection(ConnectionConfig config) {
		super(config);
	}

	/**
	 * @return true if the negotiation has been completed on this connection and
	 *         the connection is ready for file transfer
	 */
	 ...
	 * 
	 */
	public synchronized void negotiationComplete() {
	 ...
	 */
	public synchronized void clearNegotiatedStatus() {
	 ...
	 * 
	 * @return true if connection is physically connection and connection is
	 *         already negotiated and sid is assigned to this connection.
	 */
	public synchronized boolean isReady() {
		return this.isConnected && this.negotiated && this.sid != null;
	}

	/**
	 * It close the physical connection. and clear negotiated status and sid.
	 * status.
	 */
