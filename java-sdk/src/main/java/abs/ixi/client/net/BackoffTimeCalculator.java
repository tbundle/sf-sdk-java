...
public class BackoffTimeCalculator {
	 ...
	 * Fixed backoff time
	 */
	private static final long FIXED_BACKOFF = Duration.ofSeconds(10).toMillis();

	/**
	 * Max backoff time
	 */
	private static final long MAX_BACKOFF = Duration.ofMinutes(1).toMillis();

	/**
	 * Exponential base for exponential base calculations
	 */
	private static final double EXPONENTIAL_BASE = 1.25d;

	/**
	 * Backoff base value
	 */
	private static final long BASE_BACKOFF = 100;

	private BackoffPolicy policy;

	public BackoffTimeCalculator(BackoffPolicy policy) {
		this.policy = policy;
	}

	/**
	 * Calculate backoff time in millis based on given seed. Seed the number of
	 * attmpts process has already made.
	 * 
	 * @param seed number of attempts made
	 * @return backoff time in milliseconds
	 */
	public long calcBackoffTime(int seed) {
		if (BackoffPolicy.EXPONENTIAL_BACKOFF == this.policy) {
			long mills = new Double(Math.pow(seed, EXPONENTIAL_BASE)).longValue();
			return mills > MAX_BACKOFF ? MAX_BACKOFF : mills + BASE_BACKOFF;

		} else if (BackoffPolicy.FIXED_BACKOFF == this.policy) {
	...
	public BackoffPolicy getPolicy() {
		return policy;
	}
