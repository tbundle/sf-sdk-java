...
/**
 * Task executor is a wrapper around java {@link java.util.concurrent.Executor}
 * framework. Android sdk does not make use of android offered AsyncTask.
 * <p>
 * Ideally, {@code TaskExecutor} should be used for all the server interactions.
 * {@code TaskExecutor} is expected to be singleton.
 * </p>
 */
public final class TaskExecutor {
	...
	private TaskExecutor() {
		// do nothing private constructor to force use getInstance
	}

	/**
	 * Returns singleton {@link TaskExecutor} instance held
	 * 
	 * @return singleton instance of this TaskExecutor
	 */
	...
	static {
		// TODO manage core pool size and max pool size along with idle time for
		// thread timeouts
	...
	/**
	 * Submit a task to TaskExecutor for execution. Task executor does not
	 * guarantee order of execution for multiple task.
	 * 
	 * @param task {@link Runnable} to be submitted to this executor for
	 *            execution
	 */
	 ...
	 * guarantee order of execution for multiple task.
	 * 
	 * @param task {@link Task} isntance to be submitted
	 * @param <T> generic type argument of the {@link Task}
	 * @return {@link Future} instance which can be used to know execution
	 *         status of this task
	 */
	public static <T> Future<T> submit(Task<T> task) {
		return executor.submit(new TaskWrapper<T>(task));
	}
	 ...
	 * guarantee order of execution for multiple task.
	 * 
	 * @param task {@link Callable} being submitted for execution
	 * @param <V> generic param of the {@link Callable}
	 * @return {@link Future} which can be used to know the execution status of
	 *         this task
	 */
	...
	static class TaskWrapper<V> implements Callable<V> {
		private Task<V> task;

		public TaskWrapper(Task<V> task) {
			this.task = task;
		}
