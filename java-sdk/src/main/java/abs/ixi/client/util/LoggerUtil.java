...
/**
 * JUL default output handler i.e. {@link ConsoleHandler} redirects its outputs
 * to {@link System#err} stream flagging everything as error. Therefore, this
 * utility overrides that behaviour.
 */
public class LoggerUtil {
	private static final String format = "%1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS %2$s %3$s %4$s %5$s %6$s%n";

	public static final void initializeLogger(Level level) {
		...
		h.setFormatter(new SimpleFormatter() {
			...
			public synchronized String format(LogRecord lr) {
				return String.format(format, new Date(lr.getMillis()), lr.getSourceClassName(), getLogLevel(lr),
						formatMessage(lr), getExceptionMessge(lr.getThrown()), getExceptionTrace(lr.getThrown()));
			}

			private Object getExceptionMessge(Throwable thrown) {
				if (thrown != null) {
			...
			private Object getExceptionTrace(Throwable thrown) {
				if (thrown != null) {
			...
			private String getLogLevel(LogRecord lr) {
