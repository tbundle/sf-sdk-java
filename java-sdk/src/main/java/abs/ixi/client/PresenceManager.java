...
public class PresenceManager extends AbstractPacketForwarder implements PacketCollector {
	...
	public PresenceManager(XMPPStreamManager streamManager) {
		this.streamManager = streamManager;
		this.streamManager.addPacketCollector(Presence.class, this);
	}
	...
	public void collect(Packet packet) {
		...
		if (packet instanceof Presence) {
			...
			if (presence.isVCardUpdate()) {
				UserManager userManager = Platform.getInstance().getUserManager();
				userManager.getUserProfileData(presence.getFrom());
			}
		}
	}
	...
	public boolean subscribe(JID to) {
		...
		presence.setTo(to);
		presence.setType(PresenceType.SUBSCRIBE);
	...
	public boolean unsbscribe(JID to) {
		Presence presence = new Presence();
		presence.setTo(to);
		presence.setType(PresenceType.UNSUBSCRIBE);
	...
	public boolean broadcastPresence(PresenceStatus status, String mood) {
		Presence presence = new Presence();
	...
	public boolean unavailablePresence() {
		return this.unavailablePresence(null, null);
	}

	public boolean unavailablePresence(PresenceStatus status) {
		return this.unavailablePresence(status, null);
	}

	public boolean unavailablePresence(String mood) {
		return this.unavailablePresence(null, mood);
	}

	public boolean unavailablePresence(PresenceStatus status, String mood) {
		Presence presence = new Presence();
	...
	public boolean changePresenceStatus(PresenceStatus status) {
		return this.broadcastPresence(status, null);
	}
	...
	public boolean sendOfflinePresence() {
		return this.unavailablePresence();
	}
