...
public class ChatRoom implements Serializable {
	private static final long serialVersionUID = 5648332646790845393L;
	...
	private Set<RoomConfigParam> configParams;
	private boolean status;

	public ChatRoom(JID roomJID, String name, String subject) {
		this(roomJID, name, subject, true, null);
	}

	public ChatRoom(JID roomJID, String name, String subject, AccessMode accessMode) {
		this(roomJID, name, subject, true, accessMode);
	}

	public ChatRoom(JID roomJID, String name, String subject, boolean status, AccessMode accessMode) {
	...
	public ChatRoomMember getMember(JID userJID) {
	...
	public Set<RoomConfigParam> getConfigParams() {
		return configParams;
	}

	public void setConfigParams(Set<RoomConfigParam> configParams) {
		this.configParams = configParams;
	}

	public class ChatRoomMember implements Serializable {
		private static final long serialVersionUID = 1689044912643859003L;
		...
		public ChatRoomMember(JID userJID, String nickName) {
			this(userJID, nickName, true);
		}

		public ChatRoomMember(JID userJID, String nickName, Affiliation affiliation, Role role, boolean exist) {
		...
		public ChatRoomMember(JID userJID, String nickName, boolean exist) {
			this.userJID = userJID;
			this.nickName = nickName;
			this.exist = exist;
		}
	...
	public enum RoomConfigParam {
		ACCESS_MODE("muc#roomconfig_accessmode");

		private String val;

		private RoomConfigParam(String val) {
			this.val = val;
		}
		...
		public static RoomConfigParam valueFrom(String val) throws IllegalArgumentException {
			for (RoomConfigParam config : values()) {
				if (config.val().equalsIgnoreCase(val)) {
					return config;
				}
			}

			throw new IllegalArgumentException("No Room config found for value [" + val + "]");
		}
	}
}
