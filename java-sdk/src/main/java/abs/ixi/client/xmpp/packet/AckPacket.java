...
public class AckPacket extends XMPPPacket {
	private static final long serialVersionUID = 7300736049987984314L;

	private String xmlns;
	private int handledPacketCount;

	public AckPacket(String xmlns, int handledPacketCount) {
		this.xmlns = xmlns;
		this.handledPacketCount = handledPacketCount;
	}

	public AckPacket(Element element) {
		this.setXmlns(element.getAttribute("xmlns"));

		if (element.getAttribute("h") != null) {
			this.handledPacketCount = Integer.parseInt(element.getAttribute("h"));
		}
	}
	...
	@Override
	public String xml() {
		return XMPPUtils.getAckPacketString(this.handledPacketCount);
	}
}
