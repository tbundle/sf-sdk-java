...
import abs.ixi.client.xmpp.XMPPUtils;

public class StreamHeader extends XMPPPacket {
	private static final long serialVersionUID = 116371351530961333L;
	...
	public StreamHeader(boolean isClosed) {
		this.closeStream = isClosed;
	}
	...
	public StreamHeader(Element element) throws InvalidJabberId {
		...
		if (element.getAttribute("to") != null) {
			this.to = new JID(element.getAttribute("to"));
		}

		if (element.getAttribute("from") != null) {
			this.from = new JID(element.getAttribute("from"));
		}

		this.version = element.getAttribute("version");
		this.language = element.getAttribute("xml:lang");
		this.xmlns = element.getAttribute("xmlns:stream");
	}
	...
	public String xml() {
		if (closeStream) {
			return XMPPUtils.getStreamClose();

		} else {
			return XMPPUtils.getStartStream(this.from.getBareJID(), this.to.getDomain());
		}
	}
