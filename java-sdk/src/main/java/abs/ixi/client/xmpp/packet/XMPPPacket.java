...
import abs.ixi.client.core.ByteConvertible;
import abs.ixi.client.core.Packet;
import abs.ixi.client.core.XMLConvertible;
import abs.ixi.client.io.XMPPPacketReader;
import abs.ixi.client.util.StringUtils;

public abstract class XMPPPacket implements Packet, XMLConvertible, ByteConvertible {
	private static final long serialVersionUID = -6642094977846732510L;

	private static final Logger LOGGER = Logger.getLogger(XMPPPacketReader.class.getName());

	@Override
	public byte[] getBytes() {
		String packetString = this.xml();
		return StringUtils.isNullOrEmpty(packetString) ? null : packetString.getBytes(StandardCharsets.UTF_8);
	}
	...
	public long writeTo(Socket socket) throws IOException {
		return this.writeTo(socket.getOutputStream());
	}
	...
	public long writeTo(SocketChannel socketChannel) throws IOException {
		...
		if (bytes != null && bytes.length > 0) {
			return socketChannel.write(ByteBuffer.wrap(getBytes()));

		} else {
			LOGGER.info("No bytes to write");
			return 0;
		}
	}
	...
	@Override
	public String getId() {
		throw new UnsupportedOperationException("Invoked getId() operation on a packet which does not support it");
	}
