...
public class StreamFeaturePacket extends XMPPPacket {
	private static final long serialVersionUID = -1861980063615815984L;
	...
	public StreamFeaturePacket(Element element) {
		...
		for(Element child : element.getChildren()) {
			...
			if (StringUtils.safeEquals(child.getName(), XMPPTags.BIND_TAG, false)) {
				this.features.add(new ResourceBindFeature(child));
				
			} else if (StringUtils.safeEquals(child.getName(), XMPPTags.STARTTLS_TAG, false)) {
				this.features.add(new TLSFeature(child));
				
			} else if (StringUtils.safeEquals(child.getName(), XMPPTags.MECHANISMS_TAG, false)) {
				this.features.add(new SASLFeature(child));
				
			} else if (StringUtils.safeEquals(child.getName(), XMPPTags.SM_TAG, false)) {
				this.features.add(new StreamMenagementFeature(child));
			}
		}
	}
