...
public class UserSearchData implements Serializable {
	...
	public Set<String> userSearchAttributes;
	public List<Item> searchedItems;
	public Map<String, String> searchRequestData;

	public UserSearchData(Element queryElement) throws InvalidJabberId {
		if (!CollectionUtils.isNullOrEmpty(queryElement.getChildren())) {
			for (Element child : queryElement.getChildren()) {

				if (StringUtils.safeEquals(child.getName(), INSTRUCTIONS)) {
					this.instructions = child.val();

				} else if (StringUtils.safeEquals(child.getName(), FIRST)) {
					if (this.userSearchAttributes == null) {
						this.userSearchAttributes = new HashSet<>();
					}

					this.userSearchAttributes.add(FIRST);

				} else if (StringUtils.safeEquals(child.getName(), LAST)) {
					if (this.userSearchAttributes == null) {
						this.userSearchAttributes = new HashSet<>();
					}

					this.userSearchAttributes.add(LAST);

				} else if (StringUtils.safeEquals(child.getName(), NICK)) {
					if (this.userSearchAttributes == null) {
						this.userSearchAttributes = new HashSet<>();
					}

					this.userSearchAttributes.add(NICK);
					...
					this.userSearchAttributes.add(EMAIL);

				} else if (StringUtils.safeEquals(child.getName(), ITEM)) {
					if (this.searchedItems == null) {
						this.searchedItems = new ArrayList<>();
					}

					this.searchedItems.add(new Item(child));
				}
			}
		}
	}
	...
	public class Item implements Serializable {
		...
		public Item(Element itemElm) throws InvalidJabberId {
			String jid = itemElm.getAttribute(JID_ATTRIBUTE);

			if (!StringUtils.isNullOrEmpty(jid)) {
				this.userJID = new JID(jid);
			}

			if (!CollectionUtils.isNullOrEmpty(itemElm.getChildren())) {
				for (Element child : itemElm.getChildren()) {

					if (StringUtils.safeEquals(child.getName(), FIRST)) {
						this.setFirstName(child.val());

					} else if (StringUtils.safeEquals(child.getName(), LAST)) {
						this.setLastName(child.val());

					} else if (StringUtils.safeEquals(child.getName(), NICK)) {
						this.setNickName(child.val());

					} else if (StringUtils.safeEquals(child.getName(), EMAIL)) {
						this.setEmail(child.val());
