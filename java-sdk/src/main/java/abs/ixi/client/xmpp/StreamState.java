...
/**
 * {@code StreamState} indicates one of the stream states from Stream
 ...
 * can be accepted/processed by any side of the entity.
 * 
...
public enum StreamState {
	INITIATED {
		@Override
		public void setState(StreamContext ctx, Packet packet) {
			if (packet instanceof StreamHeader) {
				return;
				...
				ctx.setState(StreamState.UNKNOWN);
			}
		}
	},
	...
	STARTED {
		@Override
		public void setState(StreamContext ctx, Packet packet) {
			if (packet instanceof StreamFeaturePacket) {
				...
				for (StreamFeature feature : streamFeaturePacket.getFeatures()) {
					if (!(feature.getFeatureType() == StreamFeatureType.TLS
							|| feature.getFeatureType() == StreamFeatureType.SASL)) {
						ctx.setState(StreamState.UNKNOWN);
					}
				}
	...
	TLS_DONE {
		...
		public void setState(StreamContext ctx, Packet packet) {
			if (packet instanceof StreamHeader) {
				...
				for (StreamFeature feature : streamFeaturePacket.getFeatures()) {
					if (feature.getFeatureType() != StreamFeatureType.SASL) {
						ctx.setState(StreamState.UNKNOWN);
	...
	SASL_DONE {
		...
		public void setState(StreamContext ctx, Packet packet) {
			if (packet instanceof StreamHeader) {
				...
				for (StreamFeature feature : streamFeaturePacket.getFeatures()) {
					if (!(feature.getFeatureType() == StreamFeatureType.BIND
							|| feature.getFeatureType() == StreamFeatureType.SM)) {
	...
	SM_ENABLING {
		@Override
		public void setState(StreamContext ctx, Packet packet) {
			if (packet instanceof SMEnabledPacket) {
				return;
	...
	STREAM_RESUMING {
		@Override
		public void setState(StreamContext ctx, Packet packet) {
			if (packet instanceof StreamResumed) {
				return;
