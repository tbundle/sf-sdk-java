...
public class SMEnable extends XMPPPacket {
	private static final long serialVersionUID = -3882687526211018368L;

	private static final String SM_ENABLE_XML_WITH_MAX = "<enable xmlns='urn:xmpp:sm:3' resume ='true' max='%s'/>";
	private static final String SM_ENABLE_XML = "<enable xmlns='urn:xmpp:sm:3' resume ='true'/>";

	private long maxResumptionTimeInSec;

	public SMEnable(long maxResumptionTimeInSec) {
		this.maxResumptionTimeInSec = maxResumptionTimeInSec;
	}

	public long getMaxResumptionTimeInSec() {
		return maxResumptionTimeInSec;
	}
	...
	@Override
	public String xml() {
		if (this.maxResumptionTimeInSec == 0) {
			return SM_ENABLE_XML;

		} else {
			return String.format(SM_ENABLE_XML_WITH_MAX, maxResumptionTimeInSec);
		}
	}
}
