...
public class SASLFeature implements StreamFeature {
	...
	public SASLFeature(Element element) {
		this.xmlns = element.getAttribute("xmlns");
		
		if(!CollectionUtils.isNullOrEmpty(element.getChildren())) {
			List<SASLMechanism> mechanisms = new ArrayList<>();
			
			for(Element child : element.getChildren()) {
				
				if(StringUtils.safeEquals(child.getName(), XMPPTags.MECHANISM_TAG)) {
					mechanisms.add(SASLMechanism.valueOf(child.val()));
				}
			}
	...
	public void setMmechanisms(List<SASLMechanism> mechanisms) {
		this.mechanisms = mechanisms;
	}
