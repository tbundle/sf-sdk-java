...
public class SMEnabledPacket extends XMPPPacket {
	private static final long serialVersionUID = 6265378945521555772L;

	private String xmlns;
	private String smId;
	private boolean resumable;
	private int maxResumptionTimeInSec;

	public SMEnabledPacket(Element element) {
		this.xmlns = element.getAttribute("xmlns");

		this.smId = element.getAttribute("id");

		String resume = element.getAttribute("resume");
		if (StringUtils.safeEquals(resume, "true", false)) {
			this.resumable = true;
		}

		String maxTime = element.getAttribute("max");
		if (!StringUtils.isNullOrEmpty(maxTime)) {
			maxResumptionTimeInSec = Integer.parseInt(maxTime);
		}
	}
