...
public class XMPPError extends XMPPPacket {
	private static final long serialVersionUID = -4469337612830786288L;
	...
	public XMPPError(Element element) {
		this.error = StreamError.STREAM_ERROR;

		if (!CollectionUtils.isNullOrEmpty(element.getChildren())) {
			this.message = element.getChildren().get(0).getName();
		}
	}
	...
	public enum StreamError {
		STREAM_CLOSED, STREAM_ERROR, STREAM_NEGOTIATION_FAILED, TLS_FAILED, SASL_FAILED, UNEXPECTED_DATA, XMPP_PROTOCOL_VOILATION;
	}
