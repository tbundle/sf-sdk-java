...
/**
 * A factory to instantiate {@link XMPPPacket}. Factory takes {@link Element} as
 * an input and generates {@link XMPPPacket}
 */
public class PacketFactory {
	public static final XMPPPacket createPacket(Element element) throws Exception {
		switch (element.getName()) {

		case XMPPTags.MESSAGE_TAG:
			return new Message(element);

		case XMPPTags.ACK_TAG:
			return new AckPacket(element);

		case XMPPTags.ACK_REQUEST_TAG:
			return new AckRequestPacket(element);

		case XMPPTags.PRESENCE_TAG:
			return new Presence(element);

		case XMPPTags.IQ_TAG:
			return new IQ(element);

		case XMPPTags.STREAM_TAG:
			return new StreamHeader(element);

		case XMPPTags.STREAM_ERROR_TAG:
			return new XMPPError(element);

		case XMPPTags.STREAM_FEATURE_TAG:
			return new StreamFeaturePacket(element);

		case XMPPTags.FAILURE_TAG:
			return new Failure(element);

		case XMPPTags.SUCCESS_TAG:
			return new Success(element);

		case XMPPTags.PROCEED_TAG:
			return new Proceed(element);

		case XMPPTags.CHALLENGE_TAG:
			return new Challenge(element);

		case XMPPTags.SM_ENABLED_TAG:
			return new SMEnabledPacket(element);

		case XMPPTags.SM_RESUMED_TAG:
			return new StreamResumed(element);

		case XMPPTags.FAILED_TAG:
			return new FailedPacket(element);
