...
/**
 * {@code PacketForwarder} interface enables hierarchical packet bubble up
 * process. In order to receive packets, any {@link PacketCollector} can
 * subscribe to a {@code PacketForwarder}. A {@link PacketForwarder} forwards
 * each packet to all the subscribed {@link PacketCollector}s.
 * 
 * <p>
 * {@link PacketCollector} and {@code PacketForwarder} together can be used
 * create a hierarchical packet filtering and processing unit. A
 * {@link PacketForwarder} forwards after applying a its filters on the incoming
 * packets.
 * </p>
 */
public interface PacketForwarder {
	...
	/**
	 * Add a {@link PacketCollector} to this {@link PacketForwarder}'s list of
	 * {@link PacketCollector}s. The collector will receive all type of packets
	 * which this forwarder could not consume.
	 * 
	 * @param collector {@link PacketCollector} instance
	 */
	public void addPacketCollector(PacketCollector collector);

	/**
	 * Add a {@link PacketCollector} to this {@link PacketForwarder}'s list of
	 * {@link PacketCollector}s. This forwarder will forward the packets of type
	 * class specified to the specified collector.
	 * 
	 * @param clz {@link Class} of the packet for which collector is subscribing
	 * @param collector {@link PacketCollector} instance
	 */
	public void addPacketCollector(Class<? extends Packet> clz, PacketCollector collector);

	/**
	 * Remove the given {@link PacketCollector} from this
	 * {@link PacketForwarder} list for all packets.
	 * 
	 * @param collector {@link PacketCollector} to be removed
	 */
	public void removePacketCollector(PacketCollector collector);

	/**
	 * Remove the given {@link PacketCollector} from this
	 * {@link PacketForwarder} list. This method will remove the collector for
	 * packets of type class specified.
	 * 
	 * @param clz {@link Class} for which the collector subscription has to be
	 *            removed
	 * @param collector {@link PacketCollector} instance
	 */
	public void removePacketCollector(Class<? extends Packet> clz, PacketCollector collector);
