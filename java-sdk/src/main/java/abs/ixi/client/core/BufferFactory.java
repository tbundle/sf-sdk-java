...
/**
 * A buffer factory which recycles buffers. The factory maintains a pool of
 * {@link ByteBuffer} with default size of
 * {@link BufferFactory#BUFFER_CAPACITY}. The factory reduces frequent memory
 * allocation overheads and also helps reduce the garbage production rate.
 * 
 * @author Yogendra Sharma
 *
 */
public class BufferFactory {
	// TODO implement the buffer recycle mechanism in the factory.
	private static final int BUFFER_CAPACITY = 2048;

	/**
	 * Get a buffer of size {@link BufferFactory#BUFFER_CAPACITY}. Currently
	 * this method simply allocates the buffer of default size.
	 * 
	 * @return {@link ByteBuffer} object from factory buffer pool
	 */
	public static ByteBuffer getBuffer() {
		ByteBuffer buff = ByteBuffer.allocate(BUFFER_CAPACITY);
		return buff;
	}

	/**
	 * Get a custom sized {@link ByteBuffer} from factory. Factory does not
	 * recycle custom sized buffers.
	 * 
	 * @param size
	 *            size of the buffer
	 * @return {@link ByteBuffer} allocated
	 */
