...
/**
 * Abstract implementation of {@link PacketForwarder} interface. The class has
 * default implementation for packet forwarding and maintains collector
 * subscriptions.
 * <p>
 * {@link Set} of {@link PacketCollector}s must be synchronized before modifying
 * it as multiple thread coould use the same set.
 * </p>
 */
public class AbstractPacketForwarder implements PacketForwarder {
	private Map<Class<? extends Packet>, Set<PacketCollector>> collectors;
	private Object lock = new Object();

	protected AbstractPacketForwarder() {
	...
	public void addPacketCollector(PacketCollector collector) {
		addPacketCollector(Packet.class, collector);
	}
	...
	public void addPacketCollector(Class<? extends Packet> clz, PacketCollector collector) {
		synchronized (lock) {
			if (this.collectors.containsKey(clz)) {
				this.collectors.get(clz).add(collector);

			} else {
				Set<PacketCollector> set = new HashSet<>();
				set.add(collector);
				this.collectors.put(clz, set);
			}
		}
	}
	...
	public void removePacketCollector(PacketCollector collector) {
		for (Set<PacketCollector> collectorSet : collectors.values()) {
			synchronized (lock) {
	...
	public void removePacketCollector(Class<? extends Packet> clz, PacketCollector collector) {
		Set<PacketCollector> collectorSet = collectors.get(clz);

		synchronized (lock) {
			if (CollectionUtils.isNullOrEmpty(collectorSet)) {
				return;
			}
	...
	public void forwardPacket(Packet packet) {
		Set<PacketCollector> found = this.collectors.get(packet.getClass());

		synchronized (lock) {
			if (!CollectionUtils.isNullOrEmpty(found)) {
				for (PacketCollector collector : found) {
					collector.collect(packet);
				}
			}
		}

		forwardDefault(packet);
	}

	protected void forwardDefault(Packet packet) {
		Set<PacketCollector> found = this.collectors.get(Packet.class);
