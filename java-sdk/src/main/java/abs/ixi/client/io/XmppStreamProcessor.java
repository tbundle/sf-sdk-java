 ...
 * <p>
 * It is important to note that processing of multiplexed stream is little more
 * performance intensive than a pure XMPP stream.
 * </p>
 */
public class XmppStreamProcessor implements ByteHoldingProcessor {
	...
	public XmppStreamProcessor(PacketHandler<XMPPPacket> packetHandler) {
		...
		this.packetReceiver = new XmppPacketReceiver();
		this.networkData = new ByteSource(new byte[0]);
		this.xmppProcessor = new XMPPByteProcessor(this.packetReceiver);
	}

	public XmppStreamProcessor(PacketHandler<XMPPPacket> packetHandler, StreamContext ctx,
	...
	public void process(byte[] data, int offset, int length) throws Exception {
		...
		this.xmppProcessor.processBytes(networkData);
	}

	@Override
	public boolean hasBytePipeline() {
		return this.xmppProcessor.hasPartialBytes();
	}

	@Override
	public byte[] getBytePipeline() {
		return this.xmppProcessor.getPartialBytes();
	}

	@Override
	public void flushBytePipeline() {
		this.xmppProcessor.flushUnprocessedBytes();
	}

	/**
	 * {@link PacketReceiver} implementation to receive XMPP paackets generated
	 * by {@link XMPPByteProcessor}. {@code XmppPacketReceiver} receives packets
	 * and validates them with Protocol.
	 */
	class XmppPacketReceiver implements PacketReceiver<XMPPPacket> {

		@Override
		public boolean receive(XMPPPacket packet) throws Exception {
