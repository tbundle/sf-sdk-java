...
/**
 * Read data from a network socket. The reader works at byte level.
 */
public interface Reader extends Runnable, PacketForwarder {
	 ...
	 *         indicative of the reading process status.
	 */
	public boolean isReading();

	/**
	 * @return true if the reader process has been shutdown. A value true means
	 *         that Reader has been shutdown explicitly after it was started. A
	 *         reader which was never started
	 */
	public boolean isShutdown();

	/**
	 * Stop reader from reading. This is just setiing the stop flag so that the
	 * reader will stop in next execution of the reader loop.
	 */
	public void stop();

	/**
	 * Shutdown the reader and further reading is stopped immediately. It does
	 * not take any action on the underlying connection though.
	 * 
