...
/**
 * A facade interface to manage a XMPP stream over a connection. The interface
 * is implemented by {@link XMPPStreamManager}
 */
public interface XMPPStream extends ConnectionStateObserver {
	/**
	 * Start a stream with server with the given user credintials. The
	 * assumption is that this stream has never been started before; therefore
	 * reader, writer and connection will be used with no change.
	 * {@linkplain StreamContext} is expected to be in
	 * {@link StreamState#INITIATED} state. While the negotiation is is
	 * progress, 'negotiating' flag is turned on.
	 * 
	 * @param userName userName
	 * @param pwd pwd
	 * @param domain domain
	 * @return {@link NegotiationResult} object
	 * @throws NetworkException if there is any network related error
	 */
	public NegotiationResult startStream(String userName, String pwd, String domain) throws NetworkException;

	/**
	 * Restart a broken stream. For stream restart, underlying connection is
	 * reconnected with server and a new stream is negotiated with the server.
	 * The connection uses the previously used {@link ConnectionConfig} and user
	 * session data to restablish the connection and autentication. Essentially,
	 * the broken connection is re-established.
	 * <p>
	 * Stream negotiations are done synchronously. The method returns when
	 * stream negotiation is finished.
	 * </p>
	 * 
	 * @return NegotiationResult NegotiationResult
	 * @throws NetworkException NetworkException
	 */
	public NegotiationResult restartStream() throws NetworkException;
	...
	/**
	 * Close the ongoing stream. closing a stream does not destroy/offload any
	 * object from memory. This is a protocol level action which simply informs
	 * server to close the ongoing stream. All the objects along with physical
	 * connection with server is kept intact.
	 * 
	 * @throws NetworkException if an IO error occurrs while sending packet
	 */
