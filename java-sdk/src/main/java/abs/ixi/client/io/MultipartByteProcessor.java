...
import abs.ixi.client.io.multipart.MultipartParser;
import abs.ixi.client.io.multipart.MultipartParserContext;
import abs.ixi.client.io.multipart.MultipartParserFactory;
import abs.ixi.client.io.multipart.MultipartReader.MultipartParserCallback;
import abs.ixi.client.lang.ByteSource;

public class MultipartByteProcessor implements ByteHoldingProcessor {

	/**
	 * Singleton static MIME Parser instance
	 */
	private static final MultipartParser PARSER = MultipartParserFactory.getParser();

	/**
	 * A buffer which holds the bytes read from wire for processing
	 */
	private ByteSource networkData;

	/**
	 * MIME Parser context
	 */
	private MultipartParserContext context;

	public MultipartByteProcessor(MultipartParserCallback multipartParserCallback) {
		this.context = new MultipartParserContext(multipartParserCallback);
		this.networkData = new ByteSource(new byte[0]);
	}

	@Override
	public void process(byte[] data) throws MalformedMimeException, IOException {
		this.process(data, 0, data.length);
	}

	@Override
	public void process(byte[] data, int offset, int length) throws MalformedMimeException, IOException {
		this.networkData.reload(data, offset, (offset + length));
		PARSER.parse(networkData, context);
	}

	@Override
	public boolean hasBytePipeline() {
		return this.context.getPartialLineBytes() != null && this.context.getPartialLineBytes().length > 0;
	}

	@Override
	public byte[] getBytePipeline() {
		return this.context.getPartialLineBytes();
	}

	@Override
	public void flushBytePipeline() {
		this.context.setPartialLineBytes(null);
	}
