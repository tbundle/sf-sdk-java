...
import abs.ixi.client.file.sfcm.OutboundFileTransfer;
import abs.ixi.client.net.Connection;
import abs.ixi.client.net.SFCMConnection;
import abs.ixi.client.net.TCPConnection;

/**
 * {@code MimeUploader} uploads MIME objects/content to server. The upload
 ...
 * server.
 * <p>
 * Stringflow content model uses a separate (other than the on at which XMPP
 * stream is going on) to transfer files.
 * </p>
 * <p>
 * MIME upload may take some time (typically the objects are big in size).
 * Therefore uploader may queue up the requests. At this point, SDK use only one
 * connection to transfer mime objects. In future, we may support multiple
 * connections to speedup upload.
 * </p>
 * 
 * @author Yogi
 *
...
public class MultipartWriter {
	private static final Logger LOGGER = Logger.getLogger(MultipartWriter.class.getName());
	...
	/**
	 * Underlying network connection which is used for file transfer. In case of
	 * SFCM, it will be a raw {@link TCPConnection} at which MIME messages will
	 * be exchanged.
	 */
	private SFCMConnection connection;

	/**
	 * {@link OutboundFileTransfer} transfer request.
	 */
	private OutboundFileTransfer transfer;
	...
	public void write() throws IOException {
		LOGGER.info("writing multipart message for fileId : " + transfer.getFileId());
		this.connection.write(getMultipartMessage());
	}

	private MultipartMessage getMultipartMessage() {
		return new MultipartMessageBuilder().withMessageHeader(MEDIA_ID, transfer.getFileId())
				.withMessageHeader(STREAM_ID, xmppProtocolInstance().getStreamContext().getStreamId())
				.withMessageHeader(FROM, Platform.getInstance().getUserJID().getBareJID())
				.withMessageHeader(TO, transfer.getTo().getBareJID()).withBoundary()
				.withMediaContent(transfer.getFileId(), transfer.getFile(), transfer.getContentType()).build();
