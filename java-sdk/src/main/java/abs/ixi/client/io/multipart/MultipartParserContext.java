...
import abs.ixi.client.io.multipart.MultipartParser.ParseState;
import abs.ixi.client.lang.ByteSource;

/**
 * {@code MultipartParserContext} holds parsing process state data.
 * {@link MultipartParser} is state less class; therefore parser always parses a
 * byte stream with a context.
 */
public class MultipartParserContext {
	...
	private MimeParserCallback parserCallback;
	...
	public void addNewMultipart() {
		this.multipart = this.message.newMultipart();
	}
	...
	public void setMessageReady(boolean messageReady) {
		...
		if (messageReady) {
			parserCallback.onMimeMessage(this.message);
		}
	}

	public void mimeHeaderParsed() {
		this.parserCallback.onMimeHeader(this.message);
	}

	public void mltipartHeaderParsed() {
		this.parserCallback.onMultipartHeader(this.multipart);
	}

	public void notifyMultipartEnd() {
		this.parserCallback.onMultipart(this.multipart);
	}

	public void onMultipartContent(ByteSource byteSource, int startIndex, int length, byte[] lineBreak)
			throws IOException {

		byte[] multipartBytes = new byte[length + lineBreak.length];
		System.arraycopy(byteSource.bytes(), startIndex, multipartBytes, 0, length);
		System.arraycopy(lineBreak, 0, multipartBytes, length, lineBreak.length);

		this.parserCallback.onMultipartContent(multipartBytes, this.multipart);
	}
